...
def _http_get(url: str, *, timeout: int) -> tuple[bytes, dict]:
    req = urllib.request.Request(url, headers={"User-Agent": "mathwatch/0.1"})
...
def parse_feed(xml_bytes: bytes) -> list[dict]:
    ...
    # Some feeds wrap RSS differently; try RSS path.
...
def load_feeds(root: Path) -> list[FeedDef]:
    cfg = _read_json(root / "tools" / "mathwatch" / "feeds.json")
    feeds = []
...
def load_state(root: Path) -> dict:
    p = root / "artifacts" / "mathwatch" / "state.json"
    if not p.exists():
        return {"seen": {}}
    return _read_json(p)
...
def save_state(root: Path, state: dict) -> None:
    _write_json(root / "artifacts" / "mathwatch" / "state.json", state)
...
def cmd_fetch(args: argparse.Namespace) -> int:
    ...
    out_dir = root / "artifacts" / "mathwatch"
    ...
    for f in feeds:
        try:
            data, headers = _http_get(f.url, timeout=int(args.timeout))
        ...
        # Update seen marker to the newest item id.
        if items:
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(prog="mathwatch")
